...
from local_dist import hard_example_mining,batch_local_dist
"""
Shorthands for loss:
...
class TripletLossAligned(nn.Module):
    """Triplet loss with hard positive/negative mining.
    Reference:
    Hermans et al. In Defense of the Triplet Loss for Person Re-Identification. arXiv:1703.07737.
    Code imported from https://github.com/Cysu/open-reid/blob/master/reid/loss/triplet.py.
    ...
    """
    def __init__(self, margin=0.3):
        super(TripletLossAligned, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)
...
class TripletLossAligned1(nn.Module):
    """Triplet loss with hard positive/negative mining.
    Reference:
    Hermans et al. In Defense of the Triplet Loss for Person Re-Identification. arXiv:1703.07737.
    Code imported from https://github.com/Cysu/open-reid/blob/master/reid/loss/triplet.py.
    ...
    """
    def __init__(self, margin=0.3):
        super(TripletLossAligned1, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)
        self.ranking_loss_local = nn.MarginRankingLoss(margin=margin)
    ...
    def forward(self, inputs, targets,local_features):
        ...
        y=torch.ones_like(dist_an)
        global_loss = self.ranking_loss(dist_an,dist_ap,y)+lam*torch.mean(dist_ap)
        local_loss = self.ranking_loss_local(local_dist_an,local_dist_ap,y)+lam*torch.mean(local_dist_ap)
