...
def batch_local_dist(x, y):
    ...
    """
    assert len(x.size())==3     #3dim
    assert len(y.size())==3
    assert x.size(0)==y.size(0) #same dim
    assert x.size(-1)==y.size(-1)#same feature dim
    ...
    for i in range(m):
        for j in range(n):
            if(i==0) and (j==0):
                ...
                dist[i][j] = torch.min(dist[i][j-1],dist[i-1][j])+dist_mat[i,j]
    dist = dist[-1][-1]
    return dist
