...
from ResNet_rm import resnet,Bottleneck
from random_erasing import RandomErasing
from losses import CrossEntropyLabelSmooth, CenterLoss,TripletLoss,CrossEntropyLoss,TripletLossAligned,TripletLossAligned1
...
parser.add_argument('--resnet_rm', action='store_true', help='use th remake rennet50')
parser.add_argument('--t_c', action='store_true', help='when eval will cut a part of gallery')
parser.add_argument('--alpha',default=1.0, type=float, help='weight of trihard ')
parser.add_argument('--lam',default=0.0, type=float, help='weight of dap ')
parser.add_argument('--thr',action='store_true', help='use the remake trihardloss')
parser.add_argument('--wu',action='store_true', help='lr warm up')
parser.add_argument('--wustep',default=10, type=int, help='stepsize of warmup')
...
ax1 = fig.add_subplot(132, title="triloss")
ax2 = fig.add_subplot(133, title="crossloss")
...
def main():
    ...
    transform_train = T.Compose([
        ...
        RandomErasing(probability = args.erasing_p, mean=[0.4914, 0.4822, 0.4465]),
    ])
    ...
    if args.arch=='CNN' or args.arch=='CNN1':

        pretrained_model = models.resnet50(pretrained=True)

        
        model = Models.init_model(block = Bottleneck, layers = [3, 4, 6, 3],name=args.arch, num_classes=dataset.num_train_pids, loss={'softmax','metric'}, aligned =True)
    ...
    if args.thr:
        criterion_metric = TripletLossAligned1(margin=args.margin)
    else:
        criterion_metric = TripletLossAligned(margin=args.margin)
    ...
    if args.wu:
        scheduler = WarmupMultiStepLR(optimizer,[150,300,450],args.gamma,0.01,args.wustep,'linear')
    ...
    if args.evaluate:
        print('Evaluate Only!')
        checkpoint = torch.load('../log/densenet121_1/checkpoint_ep450.pth.tar')
    ...
    for epoch in range(start_epoch,epochs):
        ...
        train(epoch,model,criterion_class,criterion_metric,optimizer,train_loader,use_gpu,print_f)
...
def train(epoch,model,criterion_class,criterion_metric,optimizer,train_loader,use_gpu,print_f):
    model.train()
    losses = AverageMeter()
    xent_losses = AverageMeter()
    triplet_losses = AverageMeter()
    local_triplet_losses = AverageMeter()
    alpha=args.alpha
    ...
    for batch_idx, (imgs,pids,_) in enumerate(train_loader):
        ...
        triplet_loss,local_triplet_loss = criterion_metric(features,pids,local_features)
        loss = xent_loss+triplet_loss+local_triplet_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        local_triplet_losses.update(local_triplet_loss.item(),pids.size(0))
        if (batch_idx+1) % args.print_freq == 0:
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                'GLoss {triplet_loss.val:.4f} ({triplet_loss.avg:.4f})\t'
                'LLoss {local_triplet_loss.val:.4f} ({local_triplet_loss.avg:.4f})\t'.format(
                epoch+1, batch_idx+1, len(train_loader), batch_time=batch_time,
                data_time=data_time, loss=losses,xent_loss=xent_losses,triplet_loss=triplet_losses,local_triplet_loss=local_triplet_losses))

        if print_f:
            y_loss['l'].append(losses.val)
            y_loss['cl'].append(xent_losses.val)
            y_loss['tri'].append(triplet_losses.val)
            draw_curve(epoch)
            print_f=False
...
def draw_curve(current_epoch):
    ...
    ax0.plot(x_epoch, y_loss['l'], 'bo-', label='loss')
    #ax0.plot(x_epoch, y_loss['ltri'], 'bo-', label='local_triloss')
    ax1.plot(x_epoch, y_loss['tri'], 'ro-', label='triloss')
    ax2.plot(x_epoch, y_loss['cl'], 'go-', label='crossloss')
    ...
    if current_epoch == 0:
        ...
        ax1.legend()
        ax2.legend()
    fig.savefig( os.path.join(args.save_dir,'train.jpg'))
